...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    var vLength1 = Math.sqrt(Math.pow(x1,2) + Math.pow(y1, 2));
    var vLength2 = Math.sqrt(Math.pow(x2,2) + Math.pow(y2, 2));

    return Math.acos( (x1 * x2 + y1 * y2) / (vLength1 * vLength2));
}
...
function toNumber(value, def) {
    var output;
    if (typeof value === 'number' || value instanceof Number || (typeof value === 'string' && !isNaN(+value))) {
        output = value;
    } else {
        output = def;
    }
